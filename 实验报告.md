...
│  12 个页面容器   │  主题变量 + 动画   │  路由 / 状态 / 渲染  │
│  语义化结构      │  响应式 + 深色模式  │  数据管理 / 事件    │
├─────────────────┴──────────────────┴────────────────────┤
│                  localStorage 数据持久层                  │
│  campus_users │ campus_posts │ campus_messages │         │
│  campus_notifications │ campus_groups │ campus_checkins  │
│  campus_drafts │ campus_theme                            │
└─────────────────────────────────────────────────────────┘
```
...
**JS 路由逻辑：**
核心函数 `navigate(page, data)` 统一管理 12 个页面路由：首页、登录、注册、发布、动态详情、个人主页、用户主页、私信、群组、群组详情、搜索、管理员面板。流程为：隐藏当前页面 → 更新导航激活状态 → 渲染目标页面内容 → 显示目标页面。
...
*投票动态创建：*
动态添加投票选项输入框，选项数量 2-6 个。发布后生成 `type: 'poll'` 的动态对象，投票数据存储在 `pollVotes` 对象中。渲染时对每个选项生成可点击的投票卡片和 CSS 百分比条形图。
...
动态详情页复用动态卡片的 HTML 结构，额外展示完整评论列表（含用户头像、昵称、评论内容、时间戳），评论输入框支持表情和 @提及。底部固定评论输入栏。
...
function search(keyword) {
  ...
  const userResults = getAllUsers().filter(u =>
    u.id.includes(kw) || u.nickname.toLowerCase().includes(kw) ||
    u.interests.some(t => t.toLowerCase().includes(kw))
  );
  const postResults = getAllPosts().filter(p =>
